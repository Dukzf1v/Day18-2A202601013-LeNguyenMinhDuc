...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    try:
        ...
        print(f"  ⚠️  SentenceTransformer encode failed: {e}")
        return chunk_basic(text, chunk_size=300, metadata={**metadata, "strategy": "semantic"})
    ...
    for i in range(1, len(sentences)):
        ...
        norm_curr = norm(current_embedding)
        norm_next = norm(next_embedding)
        if norm_curr > 0 and norm_next > 0:
            sim = float(dot(current_embedding, next_embedding) / (norm_curr * norm_next))
        else:
            sim = 0.0
        ...
        if sim < threshold:
            ...
            current_embedding = (current_embedding * (len(current_sentences) - 1) + next_embedding) / len(current_sentences)
    ...
    for parent in parents:
        ...
        for sent in sentences:
            if len(current_child_text) + len(sent) > child_size and current_child_text:
                ...
                current_child_text = (current_child_text + " " + sent).strip()
    ...
    for line in lines:
        ...
        if header_match:
            if current_content or current_header:
                ...
                if full_text:
                    sec_name = current_header.lstrip("#").strip() if current_header else "Header"
                    chunks.append(Chunk(
                        text=full_text,
                        metadata={**metadata, "section": sec_name, "strategy": "structure", "chunk_index": len(chunks)}
                    ))
